...
var idList = yearImgCol.reduceColumns(ee.Reducer.toList(), ["year"])
                       .get("list");
idList.evaluate(function(ids) {
  print("years is", ids);
  for (var i=0; i<ids.length; i++) {
    var year = ids[i];
    var img = yearImgCol.filter(ee.Filter.eq("year", year)).first();
    //导出到 Assets
    Export.image.toAsset({
      image: img, 
      description: "Asset-chaohu-mndwi-"+year, 
      assetId: "training02/chaohu-mndwi-"+year,
      region: roi,
      scale: 30,
      crs: "EPSG:4326",
      maxPixels: 1e13
