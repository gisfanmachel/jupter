...
# Embedded file name: D:\BuildAgent\work\test/iobjectspy/ml\spacetime\_models\dcrnn\model\dcrnn_cell.py
...
class DCGRUCell(BaseModel):
    ...
    def __init__(self, input_dim, num_units, adj_mat, max_diffusion_step, num_nodes, num_proj=None, activation=torch.tanh, use_gc_for_ru=True):
        """
        ...
        supports = utils.calculate_scaled_laplacian(adj_mat, lambda_max=None)
        self._supports = self._build_sparse_matrix(supports)
        if torch.cuda.device_count() > 0:
            self._supports = self._supports.cuda()
        self.dconv_gate = DiffusionGraphConv(supports=(self._supports), input_dim=input_dim, hid_dim=num_units,
          num_nodes=num_nodes,
          max_diffusion_step=max_diffusion_step,
          output_dim=(num_units * 2))
        self.dconv_candidate = DiffusionGraphConv(supports=(self._supports), input_dim=input_dim, hid_dim=num_units,
          num_nodes=num_nodes,
          max_diffusion_step=max_diffusion_step,
    ...
    def forward(self, inputs, state):
        ...
        c = self.dconv_candidate(inputs, r * state, self._num_units)
