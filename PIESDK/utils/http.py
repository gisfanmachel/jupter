...
from .error import ConfigInitError, LoginFailError
...
def getHeaders(x_api, x_token=None):
    ...
    # 的顺序加上x - sign - headers定义的参与签名头信息按顺序取值，然后对参数值（为空不参与签名）进行拼装（即value1 & value2 & value3…）拼接成字符串tempString；
    # 2、对tempString进行MD5运算，得到32位sign；
    # 将sign作为x - sign的值放入请求头中。
...
def reLogin():
    ...
    response = GET(urlInfo=_login_url,
                   params=params,
                   headers=getHeaders(_login_url.get("x-api")))
    if response:
        _data = response.get("data", {})
