...
def _generatePIEClustererEM(pre, statement):
    """
    构造EM对象
    @param pre:
    @param statement:
...
class PIEClustererEM(PIEObject):
    def __init__(self, nClusters):
        ...
        self.pre = None
        _obj = self.getStatement(functionName="Clusterer.emConstructors",
                                 arguments={
    ...
    def train(self, features):
        ...
        _obj = self.getStatement(functionName="Clusterer.emTrain",
                                 arguments={
                                     "input": _input,
