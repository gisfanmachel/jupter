...
from .utils.common import encodeJSON
import numpy as np
...
class PIEGeometry(PIEObject):
    def __init__(self, geoJson=None, proj=None, geodesic=True, evenOdd=True):
        ...
        self.pre = None
        if geoJson and type(geoJson).__name__ == self.name():
    ...
    def MultiPoint(coords, proj):
        ...
        if coords is None: raise ArgsIsNull
        if not isinstance(coords, np.ndarray): raise ArgsIsNull
    ...
    @staticmethod
    def LineString(coords, proj, geodesic=True, maxError=None):
        """
    ...
    @staticmethod
    def MultiLineString(coords, proj, geodesic=True, maxError=None):
        """
        ...
        """
        coords = np.array(coords)
        if coords is None: raise ArgsIsNull("coords 为空")
        if not isinstance(coords, np.ndarray): raise ArgsIsNull('coords类型不对')
        if not isinstance(coords[0], np.ndarray): raise ArgsIsNull
    ...
    @staticmethod
    def LinearRing(coords, proj, geodesic=True, maxError=None):
        """
        ...
        if len(coords[0]) != 2: raise ArgsIsNull
        length = len(coords)
        if (coords[0][0] != coords[length-1][0] or coords[0][1] != coords[length-1][1]):
    ...
    @staticmethod
    def Polygon(coords, proj, geodesic=True, maxError=None, evenOdd=True):
        """
    ...
    @staticmethod
    def MultiPolygon(coords, proj, geodesic=True, maxError=None, evenOdd=True):
        """
    ...
    @staticmethod
    def Rectangle(coords, proj, geodesic=True, evenOdd=True):
        """
