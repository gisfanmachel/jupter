...
# 单张图片评价指标
def miss_fp_rate_per(tp, conf, pred_cls, target_cls,name):

    print('miss_fp_rate_per ------')
    ...
    for c in unique_classes:
        ...
        if n_p == 0 and n_gt == 0:
            ...
            if name == "each_epoch_error":   # 误检率FP/(FP+TN)
                precision_curve = tpc / (tpc + fpc)
                metric_.append(1- precision_curve[-1])
            elif name == 'each_epoch_error':  # 漏检率 1-racall
                recall_curve = tpc / (n_gt + 1e-16)
                metric_.append(1-recall_curve[-1])

    metric_ = np.array(metric_)
    if len(metric_) >0:
        mean_metrics = sum(metric_)/len(metric_)
    return mean_metrics
