...
def HVL_index (image,region):
    ...
    max = PIENumber(obj.get("B1_max"))
    min = PIENumber(obj.get("B1_min"))
    resImg = image.subtract(min).divide(max.subtract(min))
    return resImg

class PIELandsat5C2_SR(PIEObject):
    def __init__(self):
        super(PIELandsat5C2_SR, self).__init__()
        self.pre = None
        self.statement = None

    @staticmethod
    def name():
        return "PIELandsat5C2_SR"
    ...
    def NDVI(cls, input):
        ...
        red = input.select("B3")
        nir = input.select("B4")
        if red or nir is None:
            return None

        imageRed = red.multiply(0.0000275).add(-0.2)
        imageNir = nir.multiply(0.0000275).add(-0.2)
    ...
    def NDWI(cls, input):
        ...
        nir = input.select("B4")
        if green or nir is None:
            return None

        imageGreen = input.multiply(0.0000275).add(-0.2)
        imageNir = input.multiply(0.0000275).add(-0.2)
    ...
    def MNDWI(cls, input):
        ...
        imageGreen = green.multiply(0.0000275).add(-0.2)
        imageMir = nir.multiply(0.0000275).add(-0.2)
    ...
    def NDBI(cls, input):
        if input is None:
            raise ArgsIsNull("input")
        imageMIR = input.select("B5").multiply(0.0000275).add(-0.2)
        imageNir = input.select("B4").multiply(0.0000275).add(-0.2)
    ...
    def EVI(cls, input):
        if input is None:
            raise ArgsIsNull("input")
        imageRed = input.select("B3").multiply(0.0000275).add(-0.2)
        imageNir = input.select("B4").multiply(0.0000275).add(-0.2)
        imageBlue = input.select("B1").multiply(0.0000275).add(-0.2)
    ...
    def FVC(cls, input, scale):
        ...
        max = PIENumber(obj.get("ndvi_max"))
        min = PIENumber(obj.get("ndvi_min"))
    ...
    def BSI(cls, input):
        ...
        imageNir = input.select("B4")
        imageSwir = input.select("B5")
        if imageBlue is None or imageRed is None or imageSwir is None or imageNir is None:
            return None
    ...
    def imperviousSurface(cls, input, region=None, threshold=0):
        ...
        lc5Image_KT2 = ref_img.pca(lc5KTParams)
        img_TC1 = lc5Image_KT2.select("pc_0").rename('TC1')
        img_TC2 = lc5Image_KT2.select("pc_1").rename('TC2')
        img_TC3 = lc5Image_KT2.select("pc_2").rename('TC3')
...
Landsat5C2_SR = PIELandsat5C2_SR()
